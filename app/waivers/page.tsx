...
import { 
    ...
    PhoneCall, MessageCircle // 👈 استدعاء الأيقونات
} from "lucide-react";
...
export default function WaiversSearch() {
  ...
  // 🚀 2. جلب أرقام التواصل الخاصة بالشركة (نفس لوجيك صفحة اللوجين)
  useEffect(() => {
    const fetchContactInfo = async () => {
        try {
            ...
            setContactInfo({ 
                support_phone: supportPhone || "01000000000", 
                whatsapp_number: whatsappNum || "20100000000" 
            });
    ...
    <main className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans dir-rtl relative overflow-hidden pb-24 md:pb-0">
      ...
      <div className="flex-1 flex flex-col items-center p-4 relative z-10 mt-10 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-xl p-6 md:p-8 w-full border border-white/50 animate-in zoom-in-95 duration-500 mb-8">
            ...
            {/* 🚀 الكارت الأزرق الاحترافي المدمج فيه أزرار التواصل */}
            <div className="bg-blue-50/50 border border-blue-100 p-5 md:p-6 rounded-2xl mb-8 shadow-sm flex flex-col">
                ...
                {/* 💬 قسم التواصل المتغير ديناميكياً */}
                {(contactInfo.support_phone || contactInfo.whatsapp_number) && (
                    <div className="mt-5 pt-5 border-t border-blue-100/60">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <p className="text-[13px] font-black text-slate-700 text-center md:text-right">
                                هل لديك استفسار؟ <br className="md:hidden" />
                                <span className="text-xs text-slate-500 font-bold">فريق الدعم الفني يسعده مساعدتك فوراً.</span>
                            </p>
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                {contactInfo.whatsapp_number && (
                                    <a 
                                        ...
                                        className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-[#25D366]/10 text-[#25D366] h-10 px-4 rounded-xl border border-[#25D366]/30 hover:bg-[#25D366] hover:text-white shadow-sm transition-all text-xs font-black active:scale-95 group"
                                    >
                                ...
                                {contactInfo.support_phone && (
                                    <a 
                                        ...
                                        className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-100 text-slate-700 h-10 px-4 rounded-xl border border-slate-200 hover:bg-slate-900 hover:text-white shadow-sm transition-all text-xs font-black active:scale-95 group"
                                    >
