...
export default function HomeClient({ initialListings = [] }: { initialListings?: Listing[] }) {
  ...
  useEffect(() => {
    ...
    // 🚀 التعديل هنا: 300 ملي ثانية هو الأفضل لمنع إرهاق السيرفر مع الفلاتر السريعة
  ...
  return (
    // 🚀 التعديل هنا: استخدام pt-6 بدل mt-6 لمنع الفراغ الأبيض (Margin Collapse)
      <main className="min-h-screen bg-[#F8FAFC] pt-6 pb-32 text-slate-800 font-sans overflow-x-hidden dir-rtl">      
      {/* قسم البحث والفلاتر */}
      <div className="px-4 relative z-20 space-y-4 max-w-7xl mx-auto">
      ...
      <section className="px-4 relative z-10 mt-6 max-w-7xl mx-auto">
          ...
          {/* العقارات الفعلية */}
          {!loading && preparedListings.length > 0 && (
                ...
                {preparedListings.map((item) => (
                    <div key={item.id} className="transition-transform duration-300 hover:-translate-y-1.5"> 
                        <ListingCard 
                            ...
                            features={item.formattedFeatures} 
                            phone_number={item.contactPhone}
                        />
                    </div>
                ))}
              </div>
